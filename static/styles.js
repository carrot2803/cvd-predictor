document.addEventListener("DOMContentLoaded", function () {
    const styleClasses = {
        "numeric-options": "appearance-none border-2 rounded-md w-full py-2 px-3 text-gray-700 focus:outline-none",
        "selection-options": "appearance-none border-2 rounded-md w-full py-2 px-3 text-gray-700 focus:outline-none",
        "yes-no-question": "mb-4 flex flex-col sm:flex-row sm:items-center justify-between sm:space-x-3",
        "yes-no-question-label": "block text-gray-700 text-md mb-2 sm:mb-0 w-full text-left sm:w-4/5",
        "yes-no-container": "flex gap-4 sm:w-1/5",
        "yes-no-option": "w-full py-1 text-gray-700 px-3 border-2 border-gray-200 rounded-md text-center font-medium cursor-pointer peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 hover:bg-blue-600 hover:text-white hover:border-blue-600",
